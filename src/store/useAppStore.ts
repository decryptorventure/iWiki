...
function buildSeedAnalyticsEvents(userId: string): AnalyticsEvent[] {
  return [
    {
      id: 'seed-search-1',
      type: 'search',
      userId,
      query: 'quy trình onboarding',
      meta: { resultsCount: 4, source: 'dashboard' },
    ...
    },
    {
      ...
      type: 'ai_write',
      userId,
      query: 'Viết SOP onboarding nhân sự mới',
      meta: { isDoc: true },
      createdAt: daysAgoIso(1),
    },
    {
  ...
  switch (action.type) {
    ...
    case 'ADD_APPROVAL_COMMENT': {
      const updated = state.articles.map(a => {
        ...
        return {
          ...
          approval: {
            ...a.approval,
            comments: [...(a.approval?.comments || []), action.comment],
          },
          updatedAt: new Date().toISOString().split('T')[0],
        };
      });
    ...
    case 'INCREMENT_VIEWS': {
      ...
      const nextRecentReads = existing
        ? current.map((item) =>
            item.articleId === action.articleId
              ? { ...item, lastReadAt: now, count: item.count + 1 }
              : item
          )

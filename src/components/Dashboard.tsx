...
export default function Dashboard({ onSearch }: { onSearch: (q: string) => void }) {
  ...
  const { currentUser, searchHistory, recentReadsByUser } = state;
  ...
  const leaderboard = useMemo(() => {
    ...
    const base = Object.values(scoreByAuthor)
      .sort((a, b) => b.score - a.score)
      .slice(0, 5)
      .map((item, idx) => ({ ...item, rank: idx + 1 }));

    if (!base.some((item) => item.name === currentUser.name)) {
      base.splice(Math.min(4, base.length), 0, {
        rank: 0,
    ...
    <div className="max-w-7xl mx-auto px-8 py-12 relative">
      ...
      <div className="flex flex-col items-center justify-center pb-14 text-center">
        ...
        <div ref={searchRef} className={`w-full max-w-3xl relative group animate-slide-up stagger-2 ${isFocused ? 'z-50' : 'z-10'}`}>
          <div className="absolute -inset-1 bg-gradient-to-r from-[#f76226]/20 to-orange-300/20 rounded-full blur-lg opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-500"></div>
          <div className="relative">
            ...
            <div className="absolute inset-y-0 right-0 pr-4 flex items-center">
              <button
                ...
                className="p-2.5 text-white bg-gradient-to-r from-[#f76226] to-[#FF8A6A] rounded-full hover:shadow-md hover:shadow-[#f76226]/25 transition-all duration-200 active:scale-95"
                aria-label="Tìm kiếm"
              >
                <Search size={18} />
              </button>
      ...
      {/* Personalized Feed Section replaces Featured Articles to an extent, but we place it right under Search */}
          ...
          {featuredArticles[0] && (
            <div onClick={() => openArticle(featuredArticles[0].id)} className="lg:col-span-1 bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 rounded-2xl p-8 cursor-pointer hover:shadow-xl transition-all duration-300 border border-orange-100/50 flex flex-col justify-between min-h-[400px] hover:-translate-y-1">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500 mt-8">
                ...
                <span className="flex items-center gap-1"><Flame size={14} className="text-[#f76226]" /> {featuredArticles[0].likes}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-slide-up stagger-4">
              ...
              <button
                ...
                className={`px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                  allArticlesFilter === 'recent'
                    ? 'bg-[#f76226] text-white shadow-md shadow-[#f76226]/20'
        ...
        <div className="space-y-8">
          ...
          {/* For You Removed (Replaced by PersonalizedFeed above) */}
              ...
              {recentReadArticles.length === 0 ? (
                ...
                recentReadArticles.map(({ meta, article }) => (
                  <button
                    key={article.id}
                    onClick={() => openArticle(article.id)}
                    className="w-full text-left p-2.5 rounded-xl hover:bg-gray-50 transition-colors"
                  >

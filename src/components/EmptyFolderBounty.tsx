  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-900">
      ...
      <div className="px-8 py-4 border-b border-gray-100 flex items-center text-sm text-gray-500 bg-white/80 backdrop-blur-sm sticky top-0 z-10">
        <span onClick={navigateToHome} className="hover:text-[#FF6B4A] cursor-pointer transition-colors">Trang chủ</span>
        <span className="mx-2 text-gray-300">/</span>
        {breadcrumbs.map((crumb, index) => (
          <React.Fragment key={index}>
            <span className={index === breadcrumbs.length - 1 ? 'text-gray-900 font-medium' : 'hover:text-[#FF6B4A] cursor-pointer transition-colors'}>
              {crumb}
            </span>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col items-center justify-center max-w-2xl mx-auto text-center mt-10 animate-slide-up stagger-2">
          ...
          <div className="w-full relative group mt-4">
            <div className="absolute -inset-1.5 bg-gradient-to-r from-[#FF6B4A] via-orange-400 to-amber-400 rounded-2xl blur-lg opacity-30 group-hover:opacity-50 transition duration-500"></div>
            <div className="relative bg-white border border-orange-100/50 rounded-2xl p-8 shadow-md hover:shadow-xl transition-all duration-300 text-left flex flex-col sm:flex-row items-center gap-8">
              ...
              <div className="w-20 h-20 bg-gradient-to-br from-orange-50 to-amber-50 rounded-full flex items-center justify-center shrink-0 shadow-inner border border-orange-100/50 group-hover:scale-110 transition-transform duration-300">
                <Target size={36} className="text-[#FF6B4A]" />
              </div>

              <div className="flex-1 text-center sm:text-left">
                <div className="flex items-center justify-center sm:justify-start gap-3 mb-2">
                  <h4 className="text-lg font-bold text-gray-900">Nhiệm vụ: Người tiên phong</h4>
                  <span className="px-2.5 py-1 bg-gradient-to-r from-orange-100 to-amber-100 text-[#FF6B4A] text-[10px] font-bold uppercase tracking-wider rounded-full flex items-center gap-1.5 shadow-sm">
                    <Sparkles size={12} />
                    Bounty
                  </span>
                </div>
                ...
                <div className="flex items-center justify-center sm:justify-start gap-4">
                  ...
                  <button onClick={handleWriteArticle} className="flex items-center gap-2 text-sm font-semibold text-white bg-gradient-to-r from-[#FF6B4A] to-[#FF8A6A] hover:from-[#e55a2b] hover:to-[#FF6B4A] px-6 py-2.5 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg hover:shadow-[#FF6B4A]/20 active:scale-95">
                    Viết bài ngay
                    <ArrowRight size={16} />
                  </button>
                </div>

...
const STEPS_BY_ROLE: Record<OnboardingRole, OnboardingStep[]> = {
  ...
  editor: [
    ...
    },
    {
      ...
      icon: Flame,
      gradient: 'from-[#FF6B4A]/20 to-orange-500/20',
    },
  ],
  admin: [
    {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="h-1.5 bg-gray-100">
          <div
            className="h-full bg-gradient-to-r from-[#FF6B4A] to-orange-500 transition-all duration-400 ease-out"
            ...
            {steps.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-200 ${
                  i === stepIndex ? 'w-6 bg-[#FF6B4A]' : 'w-2 bg-gray-200 hover:bg-gray-300'
                }`}
                aria-label={`Bước ${i + 1}`}
              />
            ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {current.tryScreen && !isLast && onNavigate && (
              <button
                ...
                className="order-2 sm:order-1 px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-[#FF6B4A] to-orange-500 shadow-lg shadow-[#FF6B4A]/25 hover:shadow-xl transition-all flex items-center justify-center gap-2"
              >
                Thử ngay

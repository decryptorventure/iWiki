      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 animate-slide-up">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 flex items-center gap-3 mb-2">
            <div className="p-2 bg-gradient-to-br from-[#FF6B4A] to-orange-400 rounded-xl text-white shadow-md shadow-[#FF6B4A]/20">
              <FileText size={24} />
            </div>
            Bài viết của tôi
          </h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#FF6B4A] to-[#FF8A6A] text-white text-sm font-bold rounded-xl hover:shadow-lg hover:shadow-[#FF6B4A]/20 transition-all duration-200 shadow-md active:scale-95"
        >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 animate-slide-up stagger-2">
          ...
          {tabs.map(tab => {
            ...
            return (
              ...
              >
                <Icon size={16} />
                {tab.label}
                <span className={`text-xs font-bold px-1.5 py-0.5 rounded-md ${activeTab === tab.id ? 'bg-[#FF6B4A]/10 text-[#FF6B4A]' : 'bg-gray-200 text-gray-500'}`}>{tab.count}</span>
              </button>
            );
          })}
        </div>
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#FF6B4A]/20 focus:border-[#FF6B4A]/50 outline-none transition-all hover:border-gray-300"
            value={searchQuery}

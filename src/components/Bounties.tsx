...
export default function Bounties() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-12 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 animate-slide-up">
        <div>
          <h1 className="text-3xl font-extrabold text-[var(--ds-text-primary)] flex items-center gap-3 mb-2">
            <div className="p-2 bg-[var(--ds-bg-accent-primary)] rounded-xl text-white shadow-md transition-shadow"><Target size={24} /></div>
            Săn thưởng tri thức
          </h1>
          <p className="text-[var(--ds-text-secondary)]">Đóng góp tài liệu còn thiếu để nhận iKame Coins và thăng hạng.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-[var(--ds-bg-tertiary)] border border-[var(--ds-border-tertiary)] rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="bg-[var(--ds-bg-primary)] p-3 rounded-xl shadow-sm"><Trophy className="text-[var(--ds-fg-orange-strong)]" size={24} /></div>
            <div>
              <div className="text-sm text-[var(--ds-text-secondary)] font-medium">Số dư của bạn</div>
              <div className="text-2xl font-bold text-[var(--ds-text-primary)] flex items-center gap-1">{totalCoins.toLocaleString()} <Coins className="text-[var(--ds-fg-orange-strong)]" size={20} /></div>
            </div>
          </div>
      ...
      {/* Quest Banner */}
      <div className="mb-6 bg-[var(--ds-bg-primary)] border border-[var(--ds-border-secondary)] rounded-2xl p-4">
        <h3 className="font-bold text-[var(--ds-text-primary)] mb-2">Quest tuần này</h3>
        <p className="text-sm text-[var(--ds-text-secondary)]">Hoàn thành 2 bounty hoặc cập nhật 1 bài outdated để nhận thêm 200 XP + badge "Knowledge Ranger".</p>
      </div>
      ...
      {/* Bounties List */}
      {filtered.length === 0 ? (
        <div className="text-center py-20"><p className="text-[var(--ds-text-secondary)] mb-4">Không tìm thấy nhiệm vụ nào</p></div>
      ) : (
              ...
              <div key={bounty.id} className={`card-premium p-5 flex flex-col sm:flex-row sm:items-start gap-5 animate-slide-up stagger-${Math.min(i + 2, 6)}`}>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    <span className="text-xs font-medium text-[var(--ds-text-secondary)] flex items-center gap-1"><Clock size={12} /> Hạn: {formatDeadline(bounty.deadline)}</span>
                    <span className="text-xs font-medium text-[var(--ds-fg-info)] bg-[var(--ds-bg-info-subtle)] px-2 py-0.5 rounded-md flex items-center gap-1"><Users size={12} /> {bounty.acceptedBy.length} người đang làm</span>
                  </div>
                  <h3 className="text-lg font-bold text-[var(--ds-text-primary)] hover:text-[var(--ds-fg-accent-primary)] transition-colors mb-1">{bounty.title}</h3>
                  {bounty.description && <p className="text-sm text-[var(--ds-text-secondary)] mb-2 line-clamp-2">{bounty.description}</p>}
                  <p className="text-sm text-[var(--ds-text-secondary)] mb-3">Yêu cầu bởi: <span className="font-medium text-[var(--ds-text-primary)]">{bounty.requester}</span></p>
                ...
                <div className="flex sm:flex-col items-start sm:items-end justify-between gap-4 sm:gap-3 border-t sm:border-t-0 sm:border-l border-[var(--ds-border-secondary)] pt-4 sm:pt-0 sm:pl-5 shrink-0">
                  ...
                  <Button
                    ...
                    className={isAccepted ? 'text-[var(--ds-fg-success)] border-[var(--ds-border-success-subtle)] bg-[var(--ds-bg-success-subtle)] hover:bg-[var(--ds-bg-success-subtle)]' : ''}
                  >
      ...
      >
        <p className="text-sm text-[var(--ds-text-secondary)] mb-4">Mô tả rõ yêu cầu để mọi người nhận task nhanh hơn.</p>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--ds-text-primary)] mb-1">Tiêu đề nhiệm vụ *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ds-text-primary)] mb-1">Mô tả chi tiết</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ds-text-primary)] mb-1">Tags (phân cách bởi dấu phẩy)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--ds-text-primary)] mb-1">Phần thưởng (Coins) *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ds-text-primary)] mb-1">Hạn chót</label>

...
import { Button, Tabs } from '@frontend-team/ui-kit';
...
export default function AdminDashboard() {
  ...
  const tabItems = [
    {
      value: 'analytics',
      label: <div className="flex items-center gap-2"><BarChart3 size={18} /> Analytics Dashboard</div>,
      content: (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm min-h-[400px] p-6">
          <div className="flex flex-col items-center justify-center h-full py-12 text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-left mt-4 max-w-2xl">
              ...
              <Metric label="Search -> Open" value={`${baseline.searchToOpenRate}%`} />
              <Metric label="Open article" value={baseline.openArticleEvents} />
              <Metric label="AI Search" value={baseline.aiSearches} />
              <Metric label="AI Write" value={baseline.aiWrites} />
    ...
    {
      value: 'approval',
      label: <div className="flex items-center gap-2"><CheckCircle size={18} /> Phê duyệt nội dung</div>,
      content: (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm min-h-[400px] p-6">
    ...
    {
      value: 'maintenance',
      label: <div className="flex items-center gap-2"><AlertTriangle size={18} /> Bảo trì tri thức</div>,
      content: (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm min-h-[400px] p-6">
          ...
          <div className="space-y-4">
            {maintenanceArticles.map(m => (
              <div key={m.id} className="flex flex-col sm:flex-row items-start justify-between p-4 border border-red-100 bg-red-50/30 rounded-xl">
                ...
                <div className="flex gap-2 mt-4 sm:mt-0">
                  <Button variant="primary" size="s">Yêu cầu Update</Button>
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-12 animate-fade-in custom-scrollbar h-full overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <Metric label="MAU proxy (30d)" value={baseline.activeUsers30d} />
        <Metric label="Search -> Open rate" value={`${baseline.searchToOpenRate}%`} />
        <Metric label="AI Search / AI Write" value={`${baseline.aiSearches}/${baseline.aiWrites}`} />
        <Metric label="Zero-result rate" value={`${baseline.zeroResultRate}%`} />
      </div>

      <Tabs items={tabItems} value={activeTab} onValueChange={(v) => setActiveTab(v as any)} />
    </div>
  );
}

...
export default function Sidebar() {
  ...
  const [comingSoonFor, setComingSoonFor] = useState<'bounties' | 'janitor' | null>(null);
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-4">
          ...
          <div className="px-2">
            <div className="mt-1 space-y-0.5">
              ...
              <NavItem icon={Compass} label="Custom Feed" isActive={currentScreen === APP_SCREENS.CUSTOM_FEED} onClick={() => navigate(APP_SCREENS.CUSTOM_FEED)} />
              <NavItem icon={Target} label="Săn thưởng" isActive={currentScreen === APP_SCREENS.BOUNTIES} onClick={() => setComingSoonFor('bounties')} />
              <NavItem icon={AlertTriangle} label="Dọn rác dữ liệu" isActive={currentScreen === APP_SCREENS.JANITOR} onClick={() => setComingSoonFor('janitor')} />
            </div>
          </div>
      ...
      <ComingSoonModal
        open={comingSoonFor !== null}
        onOpenChange={(open) => {
          if (!open) setComingSoonFor(null);
        }}
        featureName={comingSoonFor === 'janitor' ? 'Dọn rác dữ liệu' : 'Săn thưởng'}
        description={
          comingSoonFor === 'janitor'
            ? 'Chức năng dọn rác dữ liệu sẽ có bản nâng cấp mạnh hơn. Bạn có thể xem trước Idea hiện tại.'
            : 'Chức năng săn thưởng sẽ được tối ưu để tạo nhiệm vụ và theo dõi tiến độ tốt hơn. Bạn có thể xem trước Idea hiện tại.'
        }
        onViewIdea={() => {
          if (comingSoonFor === 'janitor') {
            navigate(APP_SCREENS.JANITOR);
          } else {
            navigate(APP_SCREENS.BOUNTIES);
          }
        }}
      />
    </>
